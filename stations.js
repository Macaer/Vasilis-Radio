const stations = [
  {
    name: "Must Radio",
    stream: "https://stream.radiomustathens.com/",
    logo: "logos/must_radio.png",
    description: "House Music",
    metadataUrl: "https://stream.radiomustathens.com/status-json.xsl",
    category: "House Music"
  },
  {
    name: "Off Radio",
    stream: "https://s3.yesstreaming.net:17062/stream",
    logo: "logos/off_radio.gif",
    description: "House Music",
    metadataUrl: "https://s3.yesstreaming.net:17062/status-json.xsl",
    category: "House Music"
  },
  {
    name: "Up Radio",
    stream: "https://n04.radiojar.com/9ndpdg3c0s8uv?rj-ttl=5&rj-tok=AAABePParkkAqPgEpo_caHhURw",
    logo: "logos/up_radio.gpg",
    description: "House Music",
    metadataUrl: "https://n04.radiojar.com/9ndpdg3c0s8uv?rj-ttl=5&rj-tok=AAABePParkkAqPgEpo_caHhURw/status-json.xsl",
    category: "House Music"
  },
  {
    name: "ERT Proto",
    stream: "https://radiostreaming.ert.gr/ert-proto",
    logo: "logos/ert_proto.png",
    description: "Ειδήσεις και πολιτισμός",
    metadataUrl: "https://radiostreaming.ert.gr/status-json.xsl",
    category: "ERT Echo"
  },
  {
    name: "Athens FM",
    stream: "https://example.com/athensfm",
    logo: "logos/athens_fm.png",
    description: "Τοπικός σταθμός",
    metadataUrl: "",
    category: "FM Stations"
  },
  {
    name: "Web Chill",
    stream: "https://example.com/webchill",
    logo: "logos/web_chill.png",
    description: "Internet Chillout",
    metadataUrl: "",
    category: "Internet Radio"
  }
];